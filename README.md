...
Make sure the model weights are in place:

```bash
mkdir -p models
# or via powershell: mkdir models -ErrorAction SilentlyContinue
# Then copy or save your checkpoint:
# cp path/to/best_model_resnet_50.pth models/
```

Start the UI from project root:

```bash
streamlit run BreakHis_Classifier/resnet_app.py
```
